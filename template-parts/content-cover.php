...
<article <?php post_class(); ?> id="post-<?php the_ID(); ?>">
    ...
    <div class="home-get-started">
      ...
      <h2 class="heading-size-2">Get Started</h2>

    </div>

      <div class="home-community-posts">

        <h2 class="heading-size-2">Community</h2>

        <div class="community-posts-wrapper">

          <div class="community-posts-hero">
            ...
            foreach ($recent_posts as $post): ?>
              <a class="community-hero-img" href="<?php echo get_permalink($post['ID']) ?>" title="<?php echo $post['post_title'] ?>"><?php echo get_the_post_thumbnail($post['ID'], 'full'); ?></a>
              <div class="community-post">
                <h5><?php echo $post['post_title'] ?></h5>
                <?php the_excerpt($post['ID']); ?>
                <a class="community-link" href="<?php echo get_permalink($post['ID']) ?>" title="<?php echo $post['post_title'] ?>">Read More</a>
              </div>
              <?php
            endforeach;
            wp_reset_query(); ?>
          ...
          <div class="community-posts-secondary">
            ...
            foreach($recent_posts as $post) : ?>
              <div class="community-post">
                <a href="<?php echo get_permalink($post['ID']) ?>" title="<?php echo $post['post_title'] ?>"><?php echo get_the_post_thumbnail($post['ID'], 'full'); ?></a>
                <h5><?php echo $post['post_title'] ?></h5>
                <?php the_excerpt($post['ID']); ?>
                <a class="community-link" href="<?php echo get_permalink($post['ID']) ?>" title="<?php echo $post['post_title'] ?>">Read More</a>
              </div>
